...
void CDisc::Create(string filename, float innerRadius, float outerRadius, int curveSampleSize) {
	...
	{
		p.push_back(glm::vec3(0) + outerRadius * (float)sin(theta) + outerRadius * (float)cos(theta));


		p.push_back(glm::vec3(0) + innerRadius * (float)sin(theta) + innerRadius * (float)cos(theta));
